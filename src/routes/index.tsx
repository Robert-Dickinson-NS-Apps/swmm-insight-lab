...
import { ArrowRight, FolderTree, Network, BookOpen, GitCompare } from "lucide-react";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "SWMM5+ Repo Explorer — Overview" },
      { name: "description", content: "An interactive map of the CIMM-ORG SWMM5+ Fortran codebase, the Hodges research papers behind it, and a side-by-side mapping to EPA SWMM5 (C)." },
      { property: "og:title", content: "SWMM5+ Repo Explorer" },
      { property: "og:description", content: "Map of the SWMM5+ Fortran codebase + Hodges papers + side-by-side with EPA SWMM5 C." },
    ],
  }),
...
function OverviewPage() {
  const cards = [
    ...
    { to: "/c-alternative" as const, icon: GitCompare, title: "vs EPA SWMM5 (C)", desc: "Side-by-side: each SWMM5+ Fortran module ↔ its EPA SWMM5 C counterpart." },
  ];
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12">
      ...
      <h1 className="mt-3 text-5xl md:text-6xl leading-[1.05]">
        ...
        <span className="italic text-accent">re-engineering</span> of EPA SWMM5.
      </h1>
      <p className="mt-6 max-w-2xl text-lg text-muted-foreground leading-relaxed">
        SWMM5+ is a prototype Fortran 2008 hydraulic engine that re-uses the legacy
        EPA SWMM5 C runtime for input parsing and hydrology, and replaces the
        dynamic-wave solver with a conservative finite-volume Saint-Venant scheme
        partitioned across coarray images. This explorer maps the codebase, links
        each subsystem back to the research papers behind it, and shows how every
        Fortran module corresponds to its equivalent in the original EPA SWMM5 C source.
      </p>

      <div className="mt-10 grid grid-cols-2 gap-6 md:grid-cols-4">
        {[
          { k: "Fortran modules", v: MODULES.length },
          { k: "Subsystems", v: SUBSYSTEMS.length },
          { k: "Curated papers", v: PAPERS.length },
          { k: "C equivalents mapped", v: MODULES.filter((m) => m.cEquivalent).length },
        ].map((s) => (
